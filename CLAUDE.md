...
- **Build notes (2026-09-24, all sections built)**:
  ...
  - **Feature flag `features.showDestinations`** (`src/config/site.ts`, currently `false`): hides "اكتشف وجهاتنا" while `src/content/ar/stores.ts` still holds the Figma sample tenants. While it is off the header drops "الوجهات", the hero CTA points at `#visitors`, each category card's "اعرف المزيد" points at `#plan-your-visit`, and no anchor, sitemap entry or JSON-LD field mentions the section. Set it to `true` once real brands are in that data file; nothing else needs editing, and the section's code stays in the repo. The filter's client JavaScript sits in a shared chunk, so hiding the section does not change the 198 KB gzip client bundle.
  - **Client contact details** are real as of 2026-09-24: phone and WhatsApp `+971563775188`, email `Aldhaidvillage.ae@gmail.com`, Maps `https://maps.app.goo.gl/mJn3cftBefdqjc3E9`. Still unknown: the Instagram profile URL (`siteConfig.instagram` is `null`, which hides the footer link and drops `sameAs` from the JSON-LD) and confirmed opening hours (`siteConfig.hours` is still the Figma sample, marked TODO).
